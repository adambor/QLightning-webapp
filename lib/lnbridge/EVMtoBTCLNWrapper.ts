...
import {BTCLNtoEVMSwap, BTCLNtoEVMSwapState, IBTCLNtoEVMWrapperStorage} from "./BTCLNtoEVMWrapper";
import BTCLNtoEVMWrapper from "./BTCLNtoEVMWrapper";
...
import EVMtoBTCLN, {PaymentRequestStatus, PaymentRequestStruct} from "./EVMtoBTCLN";
import BigNumber from "bignumber.js";
import {ConstantBTCLNtoEVM, ConstantEVMtoBTCLN} from "../../src/Constants";
import {
...
export class EVMtoBTCLNSwap {
    ...
    wrapper: EVMtoBTCLNWrapper;

    events: EventEmitter;

    constructor(wrapper: EVMtoBTCLNWrapper, pr: string, data: PaymentRequestStruct, url: string, fromAddress: string);
    constructor(wrapper: EVMtoBTCLNWrapper, obj: any);

    constructor(wrapper: EVMtoBTCLNWrapper, prOrObject: string | any, data?: PaymentRequestStruct, url?: string, fromAddress?: string) {
    ...
    async refund(signer: ethers.Signer, noWaitForConfirmation?: boolean, abortSignal?: AbortSignal): Promise<TransactionResponse> {
...
export class EVMtoBTCLNWrapperLocalStorage implements IEVMtoBTCLNWrapperStorage{
    ...
    removeSwapData(swap: EVMtoBTCLNSwap): Promise<boolean> {
        this.loadIfNeeded();
    ...
    saveSwapData(swap: EVMtoBTCLNSwap): Promise<void> {
        this.loadIfNeeded();
    ...
    saveSwapDataArr(swapData: EVMtoBTCLNSwap[]): Promise<void> {
        this.loadIfNeeded();
    ...
    loadSwapData(wrapper: EVMtoBTCLNWrapper): Promise<{
        [paymentHash: string]: EVMtoBTCLNSwap
    }> {
        this.loadIfNeeded();
...
export interface IEVMtoBTCLNWrapperStorage {

    removeSwapData(swapData: EVMtoBTCLNSwap): Promise<boolean>;

    saveSwapData(swapData: EVMtoBTCLNSwap): Promise<void>;
    saveSwapDataArr(swapData: EVMtoBTCLNSwap[]): Promise<void>;
...
class EVMtoBTCLNWrapper {
    ...
    storage: IEVMtoBTCLNWrapperStorage;
    provider: ethers.providers.Provider;
    contract: EVMtoBTCLN;
    events: EventEmitter;

    swapData: {[paymentHash: string]: EVMtoBTCLNSwap};

    constructor(storage: IEVMtoBTCLNWrapperStorage, provider: ethers.providers.Provider) {
    ...
    static calculateFeeForAmount(amount: BigNumber) : BigNumber {
        return ConstantEVMtoBTCLN.baseFee.plus(amount.multipliedBy(ConstantEVMtoBTCLN.fee)).integerValue(BigNumber.ROUND_CEIL);
    }

    async create(address: string, bolt11PayRequest: string, expirySeconds: number, url: string): Promise<EVMtoBTCLNSwap> {
    ...
    async getClaimableRequests(signer: ethers.Signer): Promise<EVMtoBTCLNSwap[]> {
